...
def test_anonymous_user_cant_create_note(client, form_data):
    """Анонимный пользователь не может создать заметку и перенаправляется на логин."""
...
def test_not_unique_slug(author_client, note, form_data):
    ...
    url = reverse("notes:add")
    # Принудительно делаем слаг в форме таким же, как у существующей заметки note
    form_data["slug"] = note.slug
    notes_count_before = Note.objects.count()
    response = author_client.post(url, data=form_data)

    # Достаем форму из контекста и проверяем ошибку через assertFormError из pytest-django
...
def test_empty_slug(author_client, form_data):
    """Если слаг не заполнен, он автоматически генерируется из заголовка (title)."""
    url = reverse("notes:add")
    notes_count_before = Note.objects.count()
